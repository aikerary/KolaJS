...
const ColaBottleProgress = ({ score, maxScore, animated = true, label = "Progreso" }) => {
  ...
  return (
    <div className="flex flex-col items-center w-20 min-h-[220px]">
      {/* Etiqueta de progreso - altura fija para evitar reposicionamiento */}
      ...
      {/* Botella Container - Tamaño fijo y consistente */}
      <div className="relative w-20 h-40 flex-shrink-0 flex-grow-0">
        {/* Botella SVG - Diseño realista escalado más grande */}
        <div className="relative w-20 h-40">
          ...
          >
            {/* Definiciones para gradientes y máscara */}
            <defs>
              {/* Gradiente para el vidrio de cola */}
              <linearGradient id="colaGlassGradient" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              {/* Máscara para el líquido */}
              <mask id="liquidMask" maskUnits="userSpaceOnUse" x="7" y="21" width="46" height="91">
                <path fillRule="evenodd" clipRule="evenodd" d="M24.2689 21V36.3638C22.3876 38.6233 20.4401 40.8155 18.4418 42.9727C12.4894 49.398 8.16666 54.3731 7.99756 60.5567V109.133C8.06672 109.876 8.4024 110.573 8.94691 111.099C9.49877 111.631 10.227 111.951 10.9978 112H49.7925C50.3857 111.976 50.9948 111.701 51.3958 111.273C51.8056 110.836 52.0356 110.2 51.9981 109.608L51.9885 60.544C51.8551 54.3728 47.547 49.4062 41.5921 42.9744C39.5963 40.8188 37.6455 38.6211 35.7672 36.362V21H24.2689Z" fill="white"/>
              </mask>
            </defs>

            {/* Contorno principal de la botella */}
            <path 
          ...
          {/* Contenedor del líquido con máscara */}
          <div className="absolute inset-0">
            <svg width="80" height="160" viewBox="0 0 60 120" className="absolute inset-0">
              <defs>
                {/* Gradientes dinámicos del líquido */}
                <linearGradient id={`liquidGradient-${percentage}`} x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor={percentage >= 80 ? '#22c55e' : percentage >= 60 ? '#eab308' : percentage >= 40 ? '#f97316' : '#dc2626'} />
                  <stop offset="100%" stopColor={percentage >= 80 ? '#16a34a' : percentage >= 60 ? '#ca8a04' : percentage >= 40 ? '#ea580c' : '#b91c1c'} />
                </linearGradient>
              </defs>
              ...
              <g mask="url(#liquidMask)">
                <rect 
                  ...
                  style={{
                    ...
                    transform: `scaleY(${Math.max(animatedPercentage / 100, 0.01)})`
                  }}
                />
              </g>
            </svg>

            {/* Porcentaje dentro de la botella */}
            ...
            {/* Burbujas animadas dentro del líquido */}
            {animatedPercentage > 10 && (
              <div className="absolute inset-0" style={{ zIndex: 15 }}>
                {bubbles.slice(0, 6).map((bubble) => {
                  const bubbleBottom = 25 + (animatedPercentage / 100) * 65; // Dentro del área líquida
                  const bubbleLeft = 20 + (bubble.left / 100) * 20; // Dentro del ancho de la botella
      ...
      {/* Información del progreso - altura fija para consistencia */}
